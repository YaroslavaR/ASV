...
function [D_out] = vl_asvcovdet_y3(im, opt, frames_ori,des,isInter)
...
for rc = linspace(rc_min, rc_max, nr)
   ...
   if isInter == 1
       for in = 1:size(D,2)-1
           tempIntep1 = (2 * D(:,in,:) + D(:,in+1,:))/3;
           tempIntep2 = (D(:,in,:) + 2 * D(:,in+1,:))/3;
           D = cat(2,D,tempIntep1);
           D = cat(2,D,tempIntep2);
       end
   end
    % ------------------------------interpolation -----------------------------------
