...
function [D_out] = vl_asvcovdet_y2(im, opt, frames_ori,des,isInter)
...
for rc = linspace(rc_min, rc_max, nr)
    ...
    % spróbujmy policzy? threshold globalny
    threshold_candidates = cell(size(D,2)-1);
    threshold_final = cell(size(D,2)-1);
    for i_f = 1:nf
        for c = 1:size(D,2)-1
            ...
            threshold_candidates{c} = [threshold_candidates{c};m_candidate];
        end
        
    end
    
    for c = 1:size(D,2)-1
        threshold_final{c} = median(threshold_candidates{c});
    end
    ...
    for i_f = 1:nf
        ...
        for c = 1:size(D,2)-1
            ...
            for t = 1:size(M,2)
                ...
                accVec = accVec + (M(:,t) <= threshold_final{c});
